...
WITH TEMP_FORECASTS_DAILY AS (
    select
        ...
        (initdate_utc)::TIMESTAMP as forecast_execution_datetime
        ,initdate_utc::DATE as forecast_execution_date

        -- forecast dates
        ,validdate::DATE as forecast_date

        -- regions
        ,ref_table.region as region
        ,icao as site_id
        ...
        ,maxtemp as max_temp
        ,mintemp as min_temp
        ,avgtemp as average_temp
        ,hdd
        ,cdd

    FROM {{ source('wsi_v1', 'weighted_temp_daily_forecast_city_v2_2026_jan_12') }} f
    LEFT JOIN {{ ref('wsi_trader_city_ids_v1_2026_jan_07') }} ref_table ON f.icao = ref_table.site_id

    WHERE
        initdate_utc::DATE >= (CURRENT_TIMESTAMP AT TIME ZONE 'MST')::DATE - 10
),
