...
-- ===========================
-- ISO-NE Hourly LMPs (DA + RT + DART)
-- Grain: 1 row per date x hour_ending
-- Wide format: 1 hub x 4 components x 3 markets = 12 price columns
-- ===========================
...
WITH da AS (
    SELECT * FROM {{ ref('source_v1_isone_da_hrl_lmps') }}
),

rt AS (
    SELECT * FROM {{ ref('staging_v1_isone_rt_hrl_lmps') }}
),
