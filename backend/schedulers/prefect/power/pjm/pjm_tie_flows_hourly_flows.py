...
def pjm_tie_flows_hourly():
    """PJM 5-min tie flows — scrape latest, run dbt incremental merge to hourly mart."""
    run = pipeline_run_logger.PipelineRunLogger(
        pipeline_name="pjm_tie_flows_hourly", source="power",
    )
    run.start()
    try:
        # ────── 1. Scrape PJM 5-min tie flows and upsert to PostgreSQL ──────
        mod = importlib.import_module("backend.scrapes.power.pjm.five_min_tie_flows")
        mod.main()

        # ────── 2. Run dbt transformations (upstream only — no downstream pjm_modelling) ──────
        run_dbt("+pjm_tie_flows_hourly")
