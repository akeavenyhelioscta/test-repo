...
def _pull(
    ...
    https://www.iso-ne.com/isoexpress/web/reports/pricing/-/tree/lmps-da-hourly
        TIMESTAMP:
            10/22/2024 12:21 PM EDT
            10/21/2024 01:32 PM EDT
            10/20/2024 12:22 PM EDT

    Example:
    >>> https://www.iso-ne.com/static-transform/csv/histRpts/da-lmp/WW_DALMP_ISO_20240815.csv"
    """
    if start_date is None:
        start_date = datetime.now()

    # construct endpoint
    endpoint = f"static-transform/csv/histRpts/da-lmp/WW_DALMP_ISO_{start_date.strftime('%Y%m%d')}.csv"
    url = f"https://www.iso-ne.com/{endpoint}"

    # wait for data to be available
    response = isone_api.make_request(url, logger=logger)
