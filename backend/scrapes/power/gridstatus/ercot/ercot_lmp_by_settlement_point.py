...
# SCRAPE
API_SCRAPE_NAME = "ercot_lmp_by_settlement_point"
...
def _get_timestamp_columns(
    ...
    sced_timestamp_column: str = "SCED Timestamp",
    ...
    # sced_timestamp
    sced_timestamp = pd.to_datetime(df[sced_timestamp_column])
    sced_timestamp_local = sced_timestamp.dt.tz_localize(None)
    sced_timestamp_utc = interval_start_local - pd.Timedelta(hours=timezone_offset)
    ...
    df["sced_timestamp_local"] = sced_timestamp_local.dt.strftime(timestamp_format)
    df["sced_timestamp_utc"] = sced_timestamp_utc.dt.strftime(timestamp_format)

    # drop time
    df = df.drop(columns=[interval_start_column, interval_end_column, sced_timestamp_column])

    # After creating the timestamp columns, reorder the columns
    timestamp_columns = ['interval_start_local', 'interval_start_utc', 'interval_end_local', 'interval_end_utc', 'sced_timestamp_local', 'sced_timestamp_utc',]
...
def _format(
    ...
    # dtypes
    columns = ['location', 'location_type', 'market', 'lmp']
...
def _upsert(
    ...
    primary_key_candidates = ['interval_start_local', 'interval_start_utc', 'interval_end_local', 'interval_end_utc', 'sced_timestamp_local', 'sced_timestamp_utc', 'market', 'location', 'location_type']
...
def main(
        dates: list = [(datetime.now() - timedelta(days=1), datetime.now())],
    ):
