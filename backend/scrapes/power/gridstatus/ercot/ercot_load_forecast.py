...
def _format(
    ...
    # rename columns
    df.drop(columns=["north", "south", "west", "houston"], inplace=True)
    df.rename(columns={"system_total": "load_forecast"}, inplace=True)

    # dtypes
    columns = ["load_forecast"]
...
def main(
        dates: list = [(datetime.now(), datetime.now() + timedelta(days=7))],
    ):
