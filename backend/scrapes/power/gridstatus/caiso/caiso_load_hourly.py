...
# SCRAPE
API_SCRAPE_NAME = "caiso_load_hourly"
...
def _get_timestamp_columns(
    ...
    # drop time
    df = df.drop(columns=[interval_start_column, interval_end_column])
...
def _upsert(
    ...
    primary_keys = ['interval_start_local', 'interval_start_utc', 'interval_end_local', 'interval_end_utc', 'tac_area_name', 'load']
...
def main(
        start_date: datetime = (datetime.now() - relativedelta(days=14)),
