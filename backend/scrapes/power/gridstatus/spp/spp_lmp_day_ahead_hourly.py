...
# SCRAPE
API_SCRAPE_NAME = "spp_lmp_day_ahead_hourly"
...
def _format(
    ...
    # dtypes
    columns = ['market', 'location', 'location_type', 'pnode', 'lmp', 'energy', 'congestion', 'loss']
    for column in columns:
        if column in ['location', 'location_type', 'market', 'pnode']:
...
def _pull(
    ...
    iso = gridstatus.SPP()
    df = iso.get_lmp_day_ahead_hourly(
        date=start_date.strftime("%Y-%m-%d"),
        end=end_date.strftime("%Y-%m-%d"),
    )
    df = df[df['Location'].isin(['SPPSOUTH_HUB', 'SPPNORTH_HUB', 'SECI_SECI', 'KCPLHUB', 'OKGE_OKGE', 'NSP', 'ERCOTE', 'ERCOTN', 'EES', 'MISO', 'PJM', 'TVA', 'AECI', 'SOCO', 'SPA'])]
    df = df.reset_index(drop=True)
...
def _upsert(
    ...
    primary_keys = ['interval_start_local', 'interval_start_utc', 'interval_end_local', 'interval_end_utc', 'market', 'location', 'location_type', 'pnode']
...
def main(
        dates: list = [(datetime.now() - relativedelta(days=7), datetime.now() + relativedelta(days=1))],
    ):
