...
# SCRAPE
API_SCRAPE_NAME = "spp_solar_and_wind_forecast"
...
def _format(
    ...
    # dtypes
    columns = ['forecast_type', 'wind_forecast_mw', 'solar_forecast_mw']
    for column in columns:
        if column == 'forecast_type':
...
def _pull(
    ...
    iso = gridstatus.SPP()
    df = iso.get_solar_and_wind_forecast_mid_term(
...
def _upsert(
    ...
    primary_keys = ['interval_start_local', 'interval_start_utc', 'interval_end_local', 'interval_end_utc', 'publish_time_local', 'publish_time_utc', 'forecast_type']
