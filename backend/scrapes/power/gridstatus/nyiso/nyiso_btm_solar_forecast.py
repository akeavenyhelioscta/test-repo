...
# SCRAPE
API_SCRAPE_NAME = "nyiso_btm_solar_forecast"
...
def _get_timestamp_columns(
    ...
    # publish_time
    publish_time = pd.Series([pd.to_datetime(datetime.now().strftime(timestamp_format))] * len(df))  # Create Series with same length as df
...
def _format(
    ...
    # dtypes
    columns = ['system', 'capitl', 'centrl', 'dunwod', 'genese', 'hud_vl', 'longil', 'mhk_vl', 'millwd', 'nyc', 'north', 'west']
