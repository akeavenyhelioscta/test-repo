...
# SCRAPE
API_SCRAPE_NAME = "nav_sftp_positions_pnt_v2_2026_feb_23"

# LOGGING
LOGGING_SOURCE = "positions_and_trades"
LOGGING_PRIORITY = "high"
LOGGING_TAGS = "positions,nav"
LOGGING_TARGET_TABLE = "nav.nav_sftp_positions_pnt_v2_2026_feb_23"
...
def _read_file(
    ...
    pattern = r'Position Valuation Detail Report_(\d+)_PNT\s+Trading,\s+LLC\.(\d+)_(\d+)\.xlsx'
...
def main(
        trade_file_pattern: str = f"Position Valuation Detail Report_*_PNT TRADING, LLC.XLSX",
        lookback_days: int = 5,
        sftp_dir: Path = settings.NAV_POSITIONS_PNT_SFTP_DIR,
    ...
    try:
        ...
        filepaths = glob.glob(os.path.join(sftp_dir, "Position Valuation Detail Report_*_PNT TRADING, LLC.*.xlsx"))
