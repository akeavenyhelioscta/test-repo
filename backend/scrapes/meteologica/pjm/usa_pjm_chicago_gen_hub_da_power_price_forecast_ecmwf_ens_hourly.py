"""
Meteologica: USA PJM Chicago-GEN-HUB day ahead power price forecast ECMWF ENS hourly

Content ID: 4733
Content Name: USA PJM Chicago-GEN-HUB day ahead power price forecast ECMWF ENS hourly
...
API_SCRAPE_NAME = "usa_pjm_chicago_gen_hub_da_power_price_forecast_ecmwf_ens_hourly"
CONTENT_ID = 4733
...
# ENS00..ENS50 are renamed dynamically in _format()
...
def _format(df: pd.DataFrame, metadata: dict) -> pd.DataFrame:
    # Rename ENSxx columns to ens_xx_price
    ens_rename = {
        c: f"ens_{c[3:]}_price" for c in df.columns if c.startswith("ENS")
    }
    df = df.rename(columns=ens_rename)
    ...
    numeric_cols = ["average_price", "bottom_price", "top_price"] + [
...
def main():
    ...
    try:
        ...
        if df.empty:
            logger.info("No data returned from API -- skipping upsert.")
...
if __name__ == "__main__":
    df, metadata = _pull()
    if df.empty:
        print(f"No data returned (update_id={metadata['update_id']})")
    else:
        df = _format(df, metadata)
        _upsert(df)
