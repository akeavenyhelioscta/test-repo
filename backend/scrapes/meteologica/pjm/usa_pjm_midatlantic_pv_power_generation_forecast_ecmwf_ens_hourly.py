"""
Meteologica: USA PJM MidAtlantic photovoltaic power generation forecast ECMWF ENS hourly

Content ID: 2570
Content Name: USA PJM MidAtlantic photovoltaic power generation forecast ECMWF ENS hourly
...
API_SCRAPE_NAME = "usa_pjm_midatlantic_pv_power_generation_forecast_ecmwf_ens_hourly"
CONTENT_ID = 2570
...
def _format(df: pd.DataFrame, metadata: dict) -> pd.DataFrame:
    # Rename ENSxx columns to ens_xx_mw
    ens_rename = {
        c: f"ens_{c[3:]}_mw" for c in df.columns if c.startswith("ENS")
    }
    ...
    numeric_cols = ["average_mw", "bottom_mw", "top_mw"] + [
