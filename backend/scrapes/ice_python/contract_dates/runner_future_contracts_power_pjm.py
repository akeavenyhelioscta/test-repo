"""
ICE contract dates for PJM power futures products.

Builds the full symbol list from the PJM power futures product registry
(e.g. PMI H26-IUS, PMI J26-IUS, ...) and upserts Strip / Startdt / Enddt
...
API_SCRAPE_NAME = "runner_future_contracts_power_pjm_contract_dates"
...
def _build_symbols(
    ...
    logger.info(
        ...
        f"{len(product_codes)} PJM power futures products"
    )
    return symbols
...
def _pull(symbols: list[str]) -> list:
    if not symbols:
        logger.warning("No PJM power futures symbols built — nothing to pull")
        return []
