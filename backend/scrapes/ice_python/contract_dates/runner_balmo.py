"""
ICE contract dates for BALMO (balance-of-month) gas products.

Pulls Strip / Startdt / Enddt for every symbol in the BALMO symbol
registry and upserts to ``ice_python.contract_dates``.
"""
...
from backend.scrapes.ice_python.symbols.balmo_symbols import (
    get_balmo_symbol_codes,
)
from backend.scrapes.ice_python.contract_dates import ice_contract_dates_utils

API_SCRAPE_NAME = "runner_balmo_contract_dates"
...
def _pull(symbols: list[str]) -> list:
    if not symbols:
        logger.warning("No BALMO symbols configured — nothing to pull")
        return []
...
def main() -> pd.DataFrame:
    ...
    try:
        ...
        selected_symbols = get_balmo_symbol_codes()
        logger.info(
            f"Selected {len(selected_symbols)} BALMO symbols from "
            "backend/scrapes/ice_python/symbols/balmo_symbols.py"
        )
