"""
Runner for manually executing EIA Python scripts.
...
def discover_scripts() -> list[Path]:
    """Find all .py scripts in the EIA directory (excluding __init__, runs, flows)."""
    return sorted(
        p for p in SCRIPT_DIR.glob("*.py")
        if p.name not in ("__init__.py", "runs.py", "flows.py")
    )


def display_menu(scripts: list[Path]) -> None:
    """Print a numbered list of available EIA scripts."""
    print("\n=== Available EIA Scripts ===\n")
