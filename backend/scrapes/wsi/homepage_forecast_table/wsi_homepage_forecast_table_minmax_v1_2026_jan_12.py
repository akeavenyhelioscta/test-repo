"""
Return the WSI min/max forecast in degrees Fahrenheit for all cities in each configured pool region.
"""
...
# SCRAPE
API_SCRAPE_NAME = "wsi_homepage_forecast_table_minmax_v1_2026_jan_12"
...
def _pivot_forecast(
    ...
    return homepage_utils.pivot_forecast(
        df=df,
        normal_columns=['min_normals', 'max_normals'],
    )
...
def _pull(
    ...
    if df.empty:
        raise ValueError(f"Homepage MinMax response parsed to empty dataframe for region={region}")
