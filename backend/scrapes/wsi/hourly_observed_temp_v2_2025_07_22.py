...
# SCRAPE
# NOTE: target table name uses non-underscored date "20250722" — preserved
# for parity with existing wsi.hourly_observed_temp_v2_20250722 data.
...
def _pull(
        ...
        station_name: str,
        DataTypes: list[str] = [
            "temperature", "dewpoint", "cloudCover", "windDirection",
            "windSpeed", "heatIndex", "windChill", "relativeHumidity",
            "precipitation",
        ],
    ...
    HISTORICAL_HOURLY_OBSERVED
        - Returns observed weather data for each hour of each day between StartDate and EndDate.

    DataTypes[] (HISTORICAL_HOURLY_OBSERVED):
        temperature, dewpoint, cloudCover, windDirection, windSpeed,
        heatIndex, windChill, relativeHumidity, precipitation
    """
    ...
    params_dict = {
        ...
        "DataTypes[]": DataTypes,
        "TempUnits": "F",
        "timeutc": "false",  # local prevailing timezone (default)
    }
...
def _backfill(
    ...
    while current_date <= end_date:
        ...
        main(start_date=params['start_date'], end_date=params['end_date'])
        current_date += delta
...
def main(
    ...
    try:
        ...
        logger.section(f"Pulling hourly observed data {start_date} → {end_date}...")
        df = _pull_helper(
            start_date=start_date,
            end_date=end_date,
