...
def _get_params_string(params_dict: dict[str, Any] | None = None) -> str:
    """
    NOTE: kept for backward compatibility in scripts/docs that build explicit URLs.
    """
    if not params_dict:
        return ""
    return "&" + urlencode(params_dict, doseq=True)


def _get_wsi_trader_url(base_url: str, params_dict: dict[str, Any] | None = None) -> str:
    """
    Build a full URL with credentials and endpoint parameters.
    """
...
def _get_wsi_forecast_table_city_ids(
    ...
    df = _pull_wsi_trader_csv_data(
        base_url=base_url,
        params_dict={},
        skiprows=0,
    )
    site_ids = df.iloc[:, 0].dropna().unique().tolist()
    if not site_ids:
        raise ValueError("GetForecastTableIds returned no Site IDs.")
